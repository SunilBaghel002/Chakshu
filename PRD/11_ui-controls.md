...
|---|---|---|---|---|---|---|
| `primary` | `--signal` | none | `--signal-ink`, `--t-h2` 700 | `--signal-hot` + 120 ms diagonal sweep | fill `--signal-ink`, text `--signal` | The one viewport action. Max **one** on screen |
| `secondary` | transparent | 1 px `--line-strong` | `--ink-2`, `--t-h2` | border `--signal`, text `--signal`, bg `--signal-wash` at 6% | bg `--signal-wash` | Everything else that is a real action |
...
| `icon-ghost` | transparent | none | `--ink-3`, icon 16 | text `--signal`, bg `--panel-2` | bg `--panel-3` | Copy, close, expand. 28 × 28, hit area 40 × 40 |
| `bar` (dossier bar button) | `--signal`, `skewX(-10deg)` | none | `--signal-ink` 700 | `--signal-hot` | `--signal-ink` | SLOT-02 `DETECT CHANGES` and section headers only |
...
| `focus-visible` | 2 px `--signal` ring, `outline-offset: 2px`, radius `--r-ctl`. Never `outline: none` without replacement. Show on keyboard nav only (`:focus-visible`, not `:focus`) |
| `disabled` | opacity 0.42, `cursor: not-allowed`, no hover effect, **and a tooltip explaining why** (see §1.5) |
| `loading` | label switches to the gerund (`DETECT` → `DETECTING`), a 2 px signal indeterminate bar runs along the button's bottom edge, `aria-busy="true"`, click ignored. **No spinner icon inside a button** |
| `selected` | only for toggles/chips/tabs: `--signal-wash` fill + `--signal` border + signal text |
...
| Fill | `--well` |
| Border | 1 px `--line-strong` → `--signal` on focus |
| Radius | `--r-ctl` |
...
| Focus | 2 px signal ring, offset 1 |
...
- **K3 `select`:** native `<select>` restyled (keyboard + a11y for free), 36 px, `--well` fill, custom signal caret SVG on the right, value in `--t-body`, and the current value's unit in `--t-mono` `--ink-3` after it (`Koderi · UTM 43N`).
- **K4 `menu` (presets, sort, filter):** `--panel` popover, 1 px `--line-strong`, radius `--r-ctl`, shadow `0 8px 24px rgba(0,0,0,.55)`, `--z-tooltip`. Items 32 px, `--t-body`; hover `--panel-2`; selected item gets a 2 px signal left bar; a `--t-tag` shortcut hint right-aligned per item; dividers 1 px `--line`; max-height 320 px then scroll. Opens **below-left-aligned** to its trigger unless within 320 px of the viewport bottom, then above. Closes on `Esc`, outside click, or selection.

---

## 4. Tabs (K5)

32 px tall, gap 0, equal-width when ≤ 4 items (`flex: 1`) and content-width when more. Inactive `--ink-3` `--t-h2`; hover `--ink`; active `--signal` with a 2 px signal underline that animates from centre outwards over 160 ms (M6). A tab that would show an empty panel is disabled with a reason (§1.5), not hidden. Tabs get `role="tablist"`/`tab`/`tabpanel` and arrow-key navigation.

---

## 5. Toggles, chips, thumbs (K6, K7)

- **K6 `thumb`** (evidence triptych): 1:1 well, 1 px `--line-strong`, label `BEFORE/MASK/AFTER` in `--t-tag` overlaid bottom-left on a `rgba(8,12,22,.72)` scrim; active = signal frame + corner ticks; hover = frame `--line-strong`→`--signal` + 1.04 scale over 160 ms.
- **K7 `chip-toggle`** (year chips, filters): 28 px, `--t-tag` mono, 1 px `--line-strong`, radius `--r-tag`. Selected = `--signal-wash` fill + `--signal` border + `--signal` text. Chips are for **sets** (years, sensors, filters); radios are for **one-of** (export format). Do not use chips for a single binary state — use a `switch` (36 × 20 track, 16 px knob, `--line-strong` off / `--signal` on, 160 ms).
...
- **`status`:** 8 px dot + `--t-tag` label. ion `LIVE`, signal `DEGRADED`/`OFFLINE`, green `OK`, red `FAILED`, grey `IDLE`. The dot pulses only for `LIVE` (M8, 2 s).
- **Confidence iris:** unchanged from `ui-context.md` §5 — five arcs, ion ramp, `UNCALIBRATED` at 40% opacity with a dotted outer ring.
