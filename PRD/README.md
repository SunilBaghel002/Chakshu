...
| 9 | **[`ui-context.md`](ui-context.md)** | The design system, **v2 · Intelligence Console** (dark, amber-on-black; supersedes the v1 light theme by user directive): colour tokens with contrast ratios, the fixed slot grid (`SLOT-…` — "a place for every element"), typography, components, the map hover/animation spec M1–M10, accessibility, and copy voice. Working prototype: `../brand/ui-prototype-intel.html`. | 16 min |
...
**The positioning problem is solved by a rule, not by taste.** `ui-console.md` gives every control a slot, a size, a variant and a shortcut, and bans the three things that make interfaces drift: arbitrary spacing values, invented `z-index` numbers, and panel footers that move when content changes. The single highest-leverage rule is *one amber primary per viewport*, enforced in code by a `primaryOwner` context that downgrades a second primary and warns in dev.
