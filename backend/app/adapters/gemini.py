...
class GeminiDetectionAdapter:
    ...
    def downscale_image(self, img: Image.Image) -> tuple[Image.Image, float]:
        ...
        return img.resize(
            (round(img.width * scale), round(img.height * scale)), Image.Resampling.BILINEAR
        ), scale
    ...
    def detect(
        self, img: Image.Image, *, gsd_m: float | None, pixel_summary: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if not result:
            return {
                "status": "detection_failed",
                "error": "Gemini returned no valid detection response",
            }
        return {"status": "completed", **result}

    def _prompt(
        self, size: tuple[int, int], gsd_m: float | None, pixel_summary: dict[str, Any]
    ) -> str:
