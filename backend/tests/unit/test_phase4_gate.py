...
def test_phase4_gate_newly_built_structures_near_river(demo_tile_set: Path) -> None:
    ...
    results = service.search_semantic(
        "newly built structures near a river", filters=RetrievalFilter(limit=4)
    )
...
def test_phase4_api_semantic_get_and_post() -> None:
    ...
    post_res = client.post(
        "/api/v1/search/semantic", json={"query": "newly built structures near a river", "limit": 3}
    )
